...
FR-DIS-6: optimizer evaluates out-of-stock combinations. FR-DIS-7: restock reminders. The search as first built pinned: low ingredient counts on-hand, never bought. One shopping trip split into two answers, one budget, one ranked. Widening pool does not fix it: `canMake` counts low as makeable (FR-DIS-1, 5), so low recipe is *already* can-make; low ingredient unlocks nothing. Basket with low scores no better; ADR 15 drops it. To make low ingredient worth buying, basket scoring must move.
...
- **On**: required line short when not `in_`. Goal *fully stocked*, low ingredients join pool.
- One flag, one place in `_gapsOf`. Downstream (cross product, pool, search, shelves, rank) untouched; already reads "short".
- Low ingredient reminded by appearing in what to buy, ranked with all budget options.
...
- The optimizer's existing tests stand as the flag-off case, the low-ingredient one included — its
  subject changes from "never bought" to "not bought unless restocking".
- ADR 15's cost is cubic in the pool of ingredients worth weighing, and the switch grows that pool: a
  bar with many low ingredients pays for it. Measured over one collection of 400 recipes and 120
  ingredients at N=3, two fifths of them out and a fifth low: **58 ms plain, 100 ms restocking**. The
  growth is real but far short of cubic, the pool being bounded by the gaps' own ingredients rather
