...
and nothing crossing between. `Model` was the whole database: the store loaded it, one provider held
...
**`Shelf` above `Model`: the record of every bar, which one is open, and that one's collection —
the only one resident.**

- `Model` keeps its name and its shape. It is one bar's collection; the app gains a level rather
  than rewriting one.
...
- **Nothing crosses because there is nothing to cross to.** One `Model` is in memory, every domain
...
- **Every bar resident, `Model` per bar in one map**: switching costs nothing and the bar list is
  free. Rejected on NFR-2 — startup would decode every file, and the memoised lookups every `Model`
...
- **Rename `Model` to `Collection`**: honest, and `Bar.collection` of type `Collection` reads better
  than of type `Model`. Refused as churn: the rename touches every layer, every test and half the
  documents to say what one sentence in [components.md](../components.md#the-shelf-and-the-bar)
  says.
...
- **Shelf in the state layer, not the domain**: it is app structure, not cocktails. Refused because
  its invariants are the kind `Model`'s constructor already keeps, and the guest-bar refusal
  ([ADR 23](23-nothing-writes-a-guest-bar.md)) wants to be unit-testable without a device.

## Consequences

- The one writable provider becomes `ShelfController`; `modelProvider` survives as a derived reading
