# ADR: An ingredient answers to more than one name
...
are one ingredient but two to the vocabulary. Decided before aliases were built.

## Decision

**An ingredient carries aliases; the entry's own name stays its identity.**

- `Ingredient.aliases` — spellings an ingredient answers to, written as `aliases: [bourbon whiskey, bourbon whisky]`.
- `Collection.ingredientNamed` indexes aliases; resolution through one home.
- Uniqueness spans names and aliases under fold (ADR 08): no collisions across ingredients or within.
- References stored canonical: line with alias resolves and stores under ingredient's own name.
...
- Separate `aliases:` section: ingredient identity in two places, rename requires sync.
- Alias stored in line: file holds two names for one ingredient; resolve every read not once on load.
...
- FR-VOC-1 gains aliases, FR-ING-1 searches them, FR-VOC-6 new.
