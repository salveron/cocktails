...
- **Intent filter**: true mirror (receive Cocktails in others' sheets). Out of scope — FR-DAT-3 is inside-app action; cold-start intent + pre-load confirmation. Revisit after the pilot.
...
- **Decide import later**: ADR was first drafted this way; rejected: FR-DAT-3 and FR-DAT-1 exports are same call, one Settings menu, minute apart. A gap between the two halves would read as two different apps.
...
- **The import's mechanism is pre-chosen**: it designs the confirmation, the pre-import export and the FR-DAT-4 issue presentation.
